...
def main(args):
    ...
    if not os.path.isfile(hyperopt_file):
        # Determine whether to use subset for tuning
        if args.use_subset == 'auto':
            use_subset = not torch.cuda.is_available()
        else:
            use_subset = args.use_subset == 'True'

        if use_subset:
            print(f"\n=================== Stage 1: Tuning on ECL Subset (50 Series) ===================")
            # Slicing data to first 50 unique series to speed up hyperopt tuning by 85%
            uids = Y_df['unique_id'].unique()[:50]
            Y_df_tune = Y_df[Y_df['unique_id'].isin(uids)].copy()
        else:
            print(f"\n=================== Stage 1: Tuning on FULL ECL Dataset ({Y_df['unique_id'].nunique()} Series) ===================")
            Y_df_tune = Y_df.copy()
        ...
        trials = hyperopt_tunning(space=space,
                                  ...
                                  Y_df=Y_df_tune, X_df=X_df, S_df=S_df, f_cols=[],
        ...
        print(f"\n=================== Stage 2: Final Model Evaluation (Full Dataset) ===================")
...
def parse_args():
    ...
    parser.add_argument('--use_subset', type=str, choices=['True', 'False', 'auto'], default='auto',
                        help='Whether to use ECL subset of 50 series for tuning (default: auto, disables subsetting if GPU is available)')
